...
export default function Home() {
  ...
  const [showCookies, setShowCookies] = useState(() => localStorage.getItem("oven-cookie-consent") !== "1");
  ...
  return (
    <main dir="rtl" className="min-h-screen overflow-hidden bg-[#07111f] text-[#f7f2e8]">
      ...
      <div className="relative z-10">
        <div className="border-b border-[#d6a944]/20 bg-[#d6a944] px-4 py-2 text-center text-sm font-bold text-[#07111f] sm:text-base"><span className="ml-2 inline-flex items-center gap-1"><Sparkles size={15} /> عرض اليوم الوطني</span><span className="font-black"> خصم 15% على جميع الخدمات</span><span className="countdown-pulse mr-3 inline-flex items-center gap-1 rounded-full bg-[#07111f]/10 px-2 py-0.5 text-xs"><span>{String(countdown.days).padStart(2, "0")} يوم</span><span>:</span><span>{String(countdown.hours).padStart(2, "0")} ساعة</span><span>:</span><span>{String(countdown.minutes).padStart(2, "0")} دقيقة</span><span>:</span><span className="countdown-seconds">{String(countdown.seconds).padStart(2, "0")} ث</span></span></div>
        ...
        <section id="top" className="mx-auto grid max-w-7xl items-center gap-12 px-5 pb-20 pt-10 lg:grid-cols-[1.05fr_.95fr] lg:px-8 lg:pb-28 lg:pt-16">
          <div className="animate-fade-up">
            ...
            <p className="mt-6 max-w-xl text-lg leading-9 text-white/65 sm:text-xl">صيانة وتنظيف وإصلاح الأفران بقطع أصلية وضمان معتمد في الرياض وجدة ومكة والمدينة المنورة والدمام والخبر وأبها وتبوك. نعيد لفرنك كفاءته ونظافته بأيدي فنيين محترفين.</p>
